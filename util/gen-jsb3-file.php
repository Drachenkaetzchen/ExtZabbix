<?php
/* This script generates the zabbix.jsb3 file.
 * 
...
foreach ($records as $key => $record) {
	...
	foreach ($record->getRequires() as $require) {
		if (stripos($require, "Ext.") === false || stripos($require, "Ext.ux") !== false) {
			...
			/* As Zabbix is an application and not a class (in terms of this builder), we can't find it. Skip. */
			if ($require == "Zabbix") {
				break;
			}
	...
	if (!$bHasRequires) {
		...
		if ($record->getFilename() == "src/frontend/js/Util/i18n.js" || $record->getFilename() == "src/frontend/js/Zabbix.js" || $record->getFilename() == "src/frontend/js/Util/JsonWithAssociationsWriter.js") {
...
$template = file_get_contents(__DIR__."/../zabbix.jsb3.template");
$template = str_replace("{{FILES}}", implode(",\n", $aData), $template);

file_put_contents(__DIR__."/../zabbix.jsb3", $template);